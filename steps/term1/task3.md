...
> [!TIP]
> 学习内容用于帮助你理解整条排版链路，本次作业以“一份统一测试文稿 + 临摹主题 + 优化或重设计主题”完成对照，并不意味着需要配置全部外部服务或实际发布公众号文章。
...
2. 临摹已有的公众号文章，提取主题：
    ...
    - 临摹的目标是理解主题的构成与实现方式，而不是宣称原创；需保留参考公众号、对应文章链接和分析记录，并在 Figma 中集中展示必要的视觉对照，不得擅自将其品牌标识或专属资产用于对外发布
3. 尝试优化或重新设计公众号主题：
    ...
    - 使用与第一阶段相同的测试文稿检查两个主题，并在 Figma 中集中展示临摹版本与优化或重设计版本之间的视觉差异，说明每项主要变化解决了什么问题
...
4. 提交内容：
    - 至少包括：你临摹的公众号链接、统一测试文稿、临摹主题、优化或重设计主题、CSS、可编辑 SVG、主题所需资产、参考分析与工作记录。
    ...
    - README 明确链接统一测试文稿、两套主题源文件、工作流说明、验证记录、授权清单及具有查看权限的 Figma 文件；主题在独立项目中维护时提供本次 Commit / Tag 和具体文件入口
    - Figma 首 Page 展示两版对照并链接 README；后续用 `Final / Foundations / References & Process` 分区保存同文稿输出、排版规则、参考和主要改动。渲染结果可作为标明主题、宽度和版本的图片放入 Figma，不必重新手工排一份文章；CSS / SVG 才是对应主题与图形的可维护源文件
    - 工作流说明记录环境、启动、替换文稿、套用主题、替换颜色与 SVG、预览及交付步骤；验证记录写明两种移动端宽度、实际内容、发现的问题、修改与复查结果。可以沿用工作记录中的章节，不强制拆成多份文档
    - CSS、可编辑 SVG、测试 Markdown 和体积合适的运行图片应随主题保存；两版预览、小截图或局部差异图可以直接附在分析文档中。长文章全尺寸截图、大原图宜裁切、压缩或通过链接查看，不用截图替代可编辑 SVG，也不因控制预览体积而丢掉运行依赖
    - 注意 `.gitignore` 的规范性，不提交依赖目录、构建缓存、无必要的重复导出文件、账号密码、Cookie、Token、`.env` 或微信公众号后台凭据；图床与 Draft API 等外部服务应使用占位配置或环境变量说明

> [!NOTE]
> 这里只是希望你学一点样式与内容分离的排版基础，你想全程用秀米排版与交付也可以，但是后续的个人博客搭建时可能需要学的东西会更多。
> 如果选择秀米等替代路线，仍需使用同一测试文稿形成临摹与优化两版，提交可查看 / 复用的模板入口、套用与交付步骤、Figma 视觉对照、跨宽度验证和来源记录；平台未开放 CSS / SVG 源文件时说明限制。
> 两条路线的共同验收是内容与样式的分工、复用、视觉判断与交付；未实践的本地运行、CSS / SVG 实现知识仍保留在学习部分，后续博客任务需要时继续补齐。
