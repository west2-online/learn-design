...
>
> 你不需要成为前端工程师，但需要达到能够读懂 Agent 的产出、发现问题并精确描述需求的程度
...
> [!NOTE]
> 我们在学习部分保留了完整的博客、Agent、设计连接方式与 Skill 知识。本次作业只是以一个博客作为实践对象，必做范围以下面的作业为准，并不要求把每个工具、Skill 和技术方案都运行一遍。
>
> Agent / Web 对照、设计连接工作流探索、Skill Baseline 与改进复测可以围绕同一处真实改动展开，在一份记录中分别说明所比较的因素；并不是要为每组对照重做一个博客。
》
> 两类设计连接方式用于理解输入、输出与限制，可选择适合当前项目的一类深入实践，另一类保留为学习与比较对象，注明实际用过和仅了解的部分；Agent / Web 对照与 Skill 新场景复测仍需实际完成。
...
2. 完成个人博客的设计：
    ...
    - 在 Figma 中使用 Frame、Component 与 Variables 组织设计；视觉过程与最终设计通过可访问的 Figma 链接展示。README、对照报告与验证记录可以附体积合适的预览图、截图、WebP 或 SVG，帮助说明差异；这些附件不替代可编辑设计与实际网站
3. 使用本地 Agent 完成真实实现：
    - 将设计规则落地为集中管理的 Token / 变量和可复用组件，保持文章内容与主题样式分离
    - 至少完成三次需要读取既有项目并修改多个相关文件的 Agent 协作，每次记录任务边界、关键 Prompt、修改文件、运行命令、验收结果与人工决定
    - 三次协作可以是同一功能的连续推进，如建立组件、修复状态、完成响应式与回归；每次需有真实目标、修改和验收，不把一次过程拆成三个标题凑次数
...
7. 提交内容：
    - 可访问的个人博客链接和源码仓库链接
    - README：汇总博客定位、信息架构、技术选型、目录结构、Design Token、运行与部署、维护与迁移、资产授权和他人交接方式。若博客在独立项目仓库维护，运行、部署、维护等完整说明以项目 README 为主，考核 README 给摘要、对应章节与本次 Commit / Tag，避免维护两套全文
    - Agent 协作记录与 Agent / Web 对照报告：包含关键输入、实际修改、命令和验证、失败恢复与人工判断，不提交私密对话、凭据或无关完整日志
    - 第三方 Skill 来源与评判报告，以及优化后或自行设计的 Skill 源文件和新场景复测记录
    - README 链接 Agent 协作记录、Agent / Web 对照、Skill 来源与评判、Skill 源文件和复测、浏览器验证及维护流程、授权清单。协作记录至少能定位关键输入、修改文件 / Commit、运行结果、失败修复与人工决定；报告中涉及的视觉差异链接具体节点或标注版本的小图
    - 测试生成的截图可以择取确有说明价值、体积合适的部分提交；其余临时批量截图、长录屏和重复构建产物不必入库。较大演示视频使用合适存储的链接；本身很小的截图或录屏不因格式被禁止，仍需看总量与用途
    - 博客实际使用的压缩图片、SVG、文章和代码随项目维护，保留运行依赖；高分辨率原始素材、复杂工程等另行保存并链接，不以“都是自己的仓库”为由忽略大文件对 Git 历史的影响
