...
2. 除 task2 中学到的知识外，你还需要展示的能力如下：
    ...
    - 后期验证与交付：
        ...
        - 项目交付：
            ...
            - 暂无实现侧时，邀请同伴按交付说明走读一个任务，记录不清楚的规则与改进；组件 / Token 的实际代码映射和多尺寸运行验收留待有实现时补验，不把原型走读写成已完成开发协作
        - 验证范围说明：
            - Figma 原型主要验证流程、状态与设计意图。键盘、Focus、触控、对比、缩放等知识仍需学习和考虑，但实际运行行为只报告已在对应环境中做过的检查
            - README 和测试记录区分“设计已考虑 / 原型已验证 / 实现已验证 / 待验证”，不要求为了完成本次 Figma 作业额外开发完整软件
    - etc.
3. 提交内容：
    ...
    - 分别提供 Information Architecture（信息组织与页面层级）、User Flow（核心任务与必要分支 / 边界）和可点击 Prototype。前两者可以用思维导图 / 流程图表达，不能仅用一张“Prototype 思维导图”代替可体验的原型；README 给各自主版本位置、Prototype Flow 链接与建议体验顺序
    ...
    - Figma 首 Page 放项目摘要、最终预览、Prototype 入口、版本与 README 链接；后续用 `Flows & Prototype / Final / Foundations / Components & Assets / References & Process / Review & Handoff` 分区组织流程、正式状态、规范、组件资产、参考过程和交接标注，可以按内容合并 Pages / Sections
    - 正式界面可直接连接 Prototype，无需在“展示页”和“原型页”各维护一套相同界面。IA / Flow 若已在 Figma 维护，Markdown 只给节点链接；若使用 Mermaid 等文档维护，Figma 入口反向引用即可
    - README 汇总调研、IA / Flow、Prototype、UX 测试、交接与授权记录的链接。调研写事实 / 反馈 / 假设与来源；测试写任务、匿名参与者特征、观察、问题、修改节点及复测；交接写业务规则、状态、规格与使用方法。内容短时可以合并为 README 章节
    - README 和测试文档可以附体积合适的界面预览、关键状态或前后对照截图；自制图标、Logo / favicon 等轻量 SVG 可随授权记录提交。大分辨率整页图、全流程长录屏宜压缩或给外部链接，图片与录屏不替代可编辑设计和可点击 Prototype
